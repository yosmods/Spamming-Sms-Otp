...
def logo():
  print """%s
  __  __ ___    ___                                   
 |  \/  |   \  / __|_ __  __ _ _ __  _ __  ___ _ _ ___ %sAuthor by %𝐘𝐨𝐬-𝟗𝟗𝟗%s
 | |\/| | |) | \__ \ '_ \/ _` | '  \| '  \/ -_) '_(_-< %sGithub %sgithub.com/yosmods%s
 |_|  |_|___/  |___/ .__/\__,_|_|_|_|_|_|_\___|_| /__/ %sTeam %𝐒𝐨𝐥𝐮𝐭𝐢𝐨𝐧 𝐗 𝟗𝟗𝟗 𝐓𝐞𝐚𝐦%s
                   |_|                                 %sTools spam otp dengan 29 spammers"""%(qu,pu,ku,qu,pu,ku,qu,pu,ku,qu,qu)
#-------------------------------Input Function------------------------------------------
def input():
  global nom
  nom = raw_input("%s[%s?%s] %s𝐍𝐨𝐦𝐨𝐫 𝐓𝐚𝐫𝐠𝐞𝐭𝐧𝐲𝐚 𝐓𝐨𝐝 (8888xx) : "%(pu,me,pu,pu))
  if len(nom) < 5:
    print "%s[%s!%s] %s𝐒𝐚𝐥𝐚𝐡 𝐆𝐨𝐛𝐥𝐨𝐤 𝐁𝐮𝐤𝐚𝐧 𝐆𝐢𝐭𝐮!!"%(pu,me,pu,me)
    input()
    ...
    global jum
    jum = int(raw_input("%s[%s?%s] %s𝐉𝐮𝐦𝐥𝐚𝐡 𝐒𝐩𝐚𝐦 𝐍𝐲𝐚 𝐁𝐞𝐫𝐚𝐩𝐚 𝐓𝐨𝐝 : "%(pu,me,pu,pu)))
    main()
#-------------------------------SPAM Function-------------------------------------------
def asakita():
